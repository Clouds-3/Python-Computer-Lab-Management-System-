...
class employeeClass:
    def __init__(self,root):
        ...
        #=====title=======
        title=Label(self.root,text="Staff Details",font=("goudy old style",15),bg="#0f4d7d",fg="white").place(x=50,y=100,width=1000)
        
        
        #=====content=======
        #=====row1=====
        lbl_empid=Label(self.root,text="Stf ID",font=("goudy old style",15),bg="white").place(x=50,y=150)
        ...
        cmb_gender=ttk.Combobox(self.root,textvariable=self.var_gender,values=("Select","Male","Female","Other"),state='readonly',justify=CENTER,font=("goudy old style",13))
        ...
        cmb_utype=ttk.Combobox(self.root,textvariable=self.var_utype,values=("Admin","Staff"),state='readonly',justify=CENTER,font=("goudy old style",13))
        ...
        self.EmployeeTable=ttk.Treeview(emp_frame,columns=("sid","name","email","gender","contact","dob","doj","pass","utype","address","salary"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        scrollx.pack(side=BOTTOM,fill=X)
        scrolly.pack(side=RIGHT,fill=Y)
        scrollx.config(command=self.EmployeeTable.xview)
        scrolly.config(command=self.EmployeeTable.yview)
        
        self.EmployeeTable.heading("sid",text="S.ID")
        ...
        self.EmployeeTable.column("sid",width=90)
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get()=="":
                messagebox.showerror("Error","Staff ID you need must enter",parent=root)  
                
            else:
                cur.execute("Select * from employee where sid=?",(self.var_emp_id.get(),))    
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This Staff ID alredy assinged, try different",parent=self.root)    
                else:
                    cur.execute("Insert into employee (sid,name,email,gender,contact,dob,doj,pass,utype,address,salary) values(?,?,?,?,?,?,?,?,?,?,?)",(
                                        self.var_emp_id.get(),
                                        self.var_name.get(),
                    ...
                    con.commit()
                    messagebox.showinfo("Success","Staff Added Succesfully",parent=self.root)
                    self.show()
    ...
    def update(self):
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Invalid Staff ID",parent=self.root)    
                else:
                    cur.execute("Update employee set name=?,email=?,gender=?,contact=?,dob=?,doj=?,pass=?,utype=?,address=?,salary=? where sid=?",(
                                        self.var_name.get(),
                    ...
                    con.commit()
                    messagebox.showinfo("Success","Staff Updated Succesfully",parent=self.root)
                    self.show()
            ...
            if self.var_emp_id.get()=="":
                ...
                if row==None:
                    ...
                    if op==True:
                        ...
                        cur.execute("delete from employee where sid=?",(self.var_emp_id.get(),))
                        con.commit()
                        messagebox.showinfo("Delete","Staff Deleted Succesfully",parent=self.root)            
                        self.clear()
