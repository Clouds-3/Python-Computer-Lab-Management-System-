...
class Login_System:
    def __init__(self,root):
        ...
        lbl_user=Label(login_frame,text="Staff ID",font=('Andalus',15),bg="white",fg="#767171").place(x=50,y=100)
        ...
        lbl_reg=Label(register_frame,text="DBCY | GCC LAB",font=("times new roman",13),bg="white").place(x=100,y=15)
        #=========Animation Images=====
        self.im1=ImageTk.PhotoImage(file="images/im1.png")
        self.im2=ImageTk.PhotoImage(file="images/im4.png")
        self.im3=ImageTk.PhotoImage(file="images/im5.png")
...
#===================All Functions=============
    ...
    def login(self):
        ...
        try:
            if self.employee_id.get()=="" or self.password.get()=="":
                ...
                cur.execute("select * from employee where sid=? AND pass=?",(self.employee_id.get(),self.password.get()))
                user=cur.fetchone()
                if user==None:
    ...
    def forget_window(self):
        ...
        try:
            if self.employee_id.get()=="":
                messagebox.showerror('Error',"Staff ID must be requried",parent=self.root)
            else:
                cur.execute("select email from employee where sid=?",(self.employee_id.get(),))
                email=cur.fetchone()
                if email==None:
                    messagebox.showerror('Error',"Invalid Staff ID,try again",parent=self.root)
                else:
                    #===============Forget Window==================
    ...
    def update_password(self):
        if self.var_new_pass.get()=="" or self.var_conf_pass.get()=="":
            messagebox.showerror("Error","Password is Requried",parent=self.forget_win)
        elif self.var_new_pass.get()!= self.var_conf_pass.get():
            messagebox.showerror("Error","Please Enter Same Password !",parent=self.forget_win)
        else:
            con=sqlite3.connect(database=r'gcc.db')
            cur=con.cursor()
            try:
                cur.execute("Update employee SET pass=? where sid=?",(self.var_new_pass.get(),self.employee_id.get()))
                con.commit()
    ...
    def send_email(self,to_):
        ...
        subj='GCC LAB-Reset Password OTP'
